...
function Table() {
  ...
  this.getData = ( range ) => { // Загрузка данных
    const URL = {
      "small" : "small.json",
      "big" : "big.json"
    }
    ...
    try {
      $.getJSON( URL[range],  ( result ) => {
          ...
          this.drawContent( this.searchedData, parseInt( pages.options[ pages.selectedIndex ].value ), 0 ); 
          this.drawPagination( this.searchedData, parseInt( pages.options[ pages.selectedIndex ].value ) );
      }).fail( () => { 
  ...
  this.initToolbar = () => { // Инициализация компонентов тулбара
    searchBtn.onclick= () => this.search();
    bigDataBtn.onclick = () => this.getData( "big" );
    smallDataBtn.onclick = () => this.getData( "small" );    
    pages.onchange= () => { // Перерисовка видимого содержимого талицы при смене кол-ва страниц для вывода
      this.drawContent( this.searchedData, parseInt( pages.options[ pages.selectedIndex ].value ), 0 );
      this.drawPagination( this.searchedData, parseInt( pages.options[pages.selectedIndex ].value ) );
    } 
  }
  
  this.initHead = () => { // Инициализация заголовков (сортировка, подсветка направления сортировки)
  ...
  this.sortCol = ( colName ) => { // Сортировка кликнутого столбца
    this.visibleData.sort( ( first, next ) => { 
      if ( colName === this.lastSorted && this.reverseDir === true ) { // По убыванию
    ...
    if  ( this.lastSorted !== colName && this.reverseDir === true ) { // Условие для избежания несрабатывания сортировки при переходе от одного столбца к другому
  ...
  this.search = () => { // Поиск
    ...
    this.searchedData = this.fullData.filter( ( obj ) => { // Формируем массив данных, удовлетворяющих условиям поиска
      for ( let i=0 ; i < 5; i++ ) {
  ...
  this.drawContent = ( data, rowCount, pos ) => {  // Вывод одной страницы данных
   if ( typeof data !== "undefined" && typeof rowCount !== "undefined" && typeof pos !== "undefined" ) {
      this.visibleData = [];
   ...
   while ( tbody.firstChild ) { // Удаляем содержимое тела таблицы
      tbody.removeChild( tbody.firstChild ); 
   }
      
   this.visibleData.map( ( elem ) => {  // Загружаем новое содржимое    
        ...
        row.addEventListener( "click", () => { // Вывод дополнительной информации
          let userInfo = `<p>Выбран пользователь: <b>${elem.firstName} ${elem.lastName}</b></p><p>Описание:</p><textarea cols=40 rows=8 readonly>${elem.description}</textarea><p>Адрес проживания: <b>${elem.adress.streetAddress}</b></p><p>Город: <b>${elem.adress.city}</b></p><p>Провинция/штат: <b>${elem.adress.state}</b></p><p>Индекс: <b>${elem.adress.zip}</b></p>`;
  ...
  this.drawPagination = ( data, rowCount ) => { // Вывод пагинации
    navigation.innerHTML = "";
    for ( let i=0; i<data.length / rowCount; i++ ) { // Вычисляем кол-во страниц
